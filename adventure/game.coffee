...
(($, MITHgrid) ->
  ...
  #
  MITHgrid.namespace "Demo", (Demo) ->
    Demo.namespace "AdventureEngine", (exports) ->
    #MITHgrid.Application.namespace "AdventureEngine", (exports) ->
      exports.initInstance = (args...) ->
        console.log "AdventureEngine #initInstance"
        MITHgrid.Application.initInstance "MITHgrid.Demo.AdventureEngine", args..., (that, container) ->
          console.log "Back from constructing object", that
          ...
          that.isAtLocation = (treasure, location) ->
            ...
            if t?.environment? and location in t.environment
              true
            else
              false
          ...
          #
          that.holding = -> that.dataView.inventory.size()
    ...
    Demo.namespace "Adventure", (exports) ->
      exports.initInstance = (args...) ->
        console.log "initInstance for Adventure"
        MITHgrid.Demo.AdventureEngine.initInstance "MITHgrid.Demo.Adventure", args..., (that, container) ->
          options = that.options
          console.log "Prepping game setup"
          that.ready ->
            ...
            #
            makeLoc "crack", "The crack is far too small for you to follow."
            force   "spit"

            makeLoc "neck", "You are at the bottom of the pit with a broken neck."
            force   "limbo2"

            makeLoc "limbo2", "Game over.  Reload the page to play again."

            makeLoc "lose", "You didn't make it."
            force   "limbo2"

            makeLoc "cant", "The dome is unclimbable."
            makeInst "FORCE", 0, "emist"

            makeLoc "climb", "You clamber up the plant and scurry through the hole at the top"
            force   "narrow"
            ...
            makeLoc "snaked", "You can't get by the snake."
            force   "hmk"

            makeLoc "thru", "You have crawled through a very low wide passage parallel to and north of the Hall of Mists."
            force   "wmist"
  ...
  #
  MITHgrid.defaults "MITHgrid.Demo.AdventureEngine",
    variables:
      WasDark:
        "default": false
        is: 'rw'
